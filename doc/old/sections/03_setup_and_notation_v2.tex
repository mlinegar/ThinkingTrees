...
The previous section introduced the oracle $\fstar$ as a stand-in for expensive human judgment and motivated hierarchical summarization as a practical necessity. But it left open a central question: how can we trust a summary produced by a long pipeline when we cannot afford to run the oracle on the full document? Our answer relies on a shift in perspective. Instead of validating the final summary directly (which would require processing the full text), we treat the summarization pipeline as a stochastic process and audit its atomic operations.

\subsection{Three Consistency Conditions}

We introduce three local conditions that, when satisfied, guarantee the final summary preserves the oracle in expectation.
...
This approach can be viewed as a semantic generalization of \emph{mergeable summaries} from database theory \citep{agarwal2013mergeable}. That literature constructs algebraic data structures (sketches) that preserve commutative statistics like counts or heavy hitters under merge operations. We seek to preserve arbitrary semantic oracles (event extraction, narrative flow, structured coding) under non-commutative string concatenation. We replace the explicit algebra of sketches with learned local consistency conditions that force a generic summarizer to behave as if it were a mergeable sketch. When the oracle happens to be symmetric (e.g., counting events where order is irrelevant), our conditions collapse exactly to standard mergeability; Appendix~\ref{app:mergeable} makes this correspondence precise.
