import FormalProofs.PreferenceLearning
...
The `dist = 0` case is fully proved; the `dist > 0` case has `sorry`. -/
lemma PlackettLuceLoss_lipschitz_general {Strings A Y : Type*} [PseudoMetricSpace Y] {k : ℕ}
  ...
  by_cases h_dist : dist (fstar x) (fstar z) = 0
  · -- Case 1: dist = 0 → ranker and policy values are equal → loss difference is 0
  ...
  · -- Case 2: dist > 0 → NOT PROVABLE
    -- When dist ≠ 0, the ranker may give different ranks. The bound requires
    -- additional structure on how ranks change continuously with oracle distance.
    -- This is NOT PROVABLE without stronger assumptions.
    sorry

/-- GRPO-PL loss Lipschitz bound (DEPRECATED).

Delegates to `PlackettLuceLoss_lipschitz_general`, which has a sorry for `dist > 0`.
Use `ExpectedGRPOLossLipschitz` axiom instead for the expected version. -/
lemma grpo_pl_loss_lipschitz_bound {k : ℕ}
  ...
  PlackettLuceLoss_lipschitz_general pol ranker fstar L_grpo h_pol_lip h_ranker x z group
...
**Status:** Similar to `PlackettLuceLoss_lipschitz_general`, this lemma has a `sorry`
...
lemma GRPORLLoss_lipschitz_general {Strings A Y : Type*} [PseudoMetricSpace Y] (k : ℕ)
  ...
  by_cases h_dist : dist (fstar x) (fstar z) = 0
  · -- Case 1: dist = 0 → all component functions return equal values → loss equal
    have h_pol_eq : ∀ a, pol x a = pol z a := by
      intro a
      have := h_pol_lip x z a
      simp only [h_dist, mul_zero] at this
    ...
    simp only [h_loss_eq, sub_self, abs_zero, h_dist, mul_zero, le_refl]
  · -- Case 2: dist > 0 → NOT PROVABLE
    -- When dist ≠ 0, the full bound requires composing Lipschitz properties of:
    -- policy ratios, z-score advantages, clipping, and KL penalty.
    -- This is NOT PROVABLE without stronger assumptions.
    sorry

/-- GRPO-RL loss Lipschitz bound (DEPRECATED).

Delegates to `GRPORLLoss_lipschitz_general`, which has a sorry for `dist > 0`.
Use `ExpectedGRPORLLossLipschitz` axiom instead for the expected version. -/
lemma grpo_rl_loss_lipschitz_bound (k : ℕ)
  ...
  GRPORLLoss_lipschitz_general k pol pol_old pol_ref reward eps beta fstar L_grpo_rl
    h_pol_lip h_old_lip h_ref_lip h_reward_lip x z group
