...
token ids for HLL, hashables for Count-Min/Frequent Items/Theta, floats for
KLL/REQ/t-digest, and weighted items for Tuple/VarOpt).
...
class SketchAdapter(Protocol[StateT, ItemT, QueryT, ResultT]):
    ...
    - `merge` is associative; commonly also commutative and idempotent (HLL,
      Count-Min, Theta) — adapters declare this via `is_commutative` and
      `is_idempotent`.
