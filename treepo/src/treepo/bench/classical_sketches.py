...
class ClassicalSketchComparisonConfig:
    ...
    learned_batch_size: int = 16
    learned_learning_rate: float = 1e-3
    learned_embedding_dim: int = 32
    learned_summary_dim: int = 64
...
def _row_axes(config: ClassicalSketchComparisonConfig) -> Dict[str, object]:
    return {
...
def _attach_official_floors(rows: List[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        if str(row.get("implementation_status")) != "official_empirical":
            continue
...
def _run_frequency(config: ClassicalSketchComparisonConfig, docs: Sequence[Sequence[int]]) -> List[Dict[str, object]]:
    ...
    for adapter, status, formal_status, theory in adapters:
        ...
        mem: List[float] = []
        for doc in docs:
            counts = Counter(str(x) for x in doc)
