...
Appendix~\ref{sec:markov-walkthrough} manifesting in real data: when
...
Appendices~\ref{sec:markov-walkthrough} and~\ref{sec:hll-parity}; it uses
...
\subsection[Alternating f/g Optimization Across the Benoit Dimensions]{Alternating $f/g$ Optimization Across the Benoit
Policy Dimensions}\label{app:rile-fg-ladder}

This subsection provides the full details behind
Section~\ref{sec:rile-fg-ladder}'s main-body summary of the alternating
$f/g$ ladder: the two economic-dimension initialization lanes in
depth, then the six-dimension grid and its joint shared-summarizer
variant.
...
\paragraph{Two initialization lanes.} The economic lane runs under
two initializations:
\begin{itemize}[leftmargin=1.5em]
...
\begin{table}[htbp]
    \centering
    \resizebox*{!}{0.88\textheight}{\resizebox{\linewidth}{!}{\input{assets/benoit/tables/manifesto_fg_ladder.tex}}}
...
$+0.064$ over the few-shot optimized scorer ($0.822$). The
    ...
    system above the fixed-$g$ ablation range. The per-dimension
headline row of Table~\ref{tab:benoit-headline} ($0.939$ on economic
at $c = 8\mathrm{K}$ chars) is out of scope as a comparator: the leaf
axis units differ, and no matched-leaf fixed-prompt baseline at the
same prompt-search budget exists
(Section~\ref{sec:rile-fg-ladder}).

\subsubsection{All Six Benoit Dimensions}

The single-dimension grid runs the same alternating ladder once per
Benoit dimension: token leaves
$\{256, 512, 1024, 2048, 4096, 8192\}$, anchor $f^1 g^0$ plus five
alternation steps (summarizer side first) up to $f^3 g^3$, with all
$36$ dimension--leaf cells complete.\footnote{Per-cell histories:
\texttt{outputs/manifesto\_fg\_alternating/scalar\_dims\_benoit\_all6\_fresh8192\_dspy\_20260427\_015845/<dim>/ladder/dspy/leaf*tok/iteration\_history.json}.
Joint variant:
\texttt{outputs/manifesto\_fg\_alternating/combined\_benoit\_joint\_teacher\_all6\_dspy\_fixed\_20260425\_000122},
tidy per-dimension table in
\texttt{plots\_by\_dimension/manifesto\_fg\_ladder\_dimension\_rows.csv}.}
Table~\ref{tab:fg-ladder-perdim} reports each dimension's best
held-out (test) external Pearson with the leaf size and ladder stage
at which it occurs.
...
\begin{table}[t]
    ...
    \begin{tabular}{lccc}
        \toprule
        Dimension & Best test $r$ & Leaf (tokens) & Stage \\
        \midrule
        Economic             & $0.897$ & $4096$ & $f^2 g^1$ \\
        Social               & $0.928$ & $2048$ & $f^2 g^2$ \\
        Immigration          & $0.894$ & $4096$ & $f^1 g^1$ \\
        European integration & $0.971$ & $1024$ & $f^2 g^1$ \\
        Environment          & $0.779$ & $8192$ & $f^1 g^1$ \\
        Decentralization     & $0.540$ & $256$  & $f^1 g^0$ \\
        \bottomrule
    \end{tabular}
    \caption{Single-dimension alternating ladder across all six
    Benoit dimensions: best held-out external Pearson $r$ against
    the Benoit expert mean per dimension, with the token leaf size
    and ladder stage of the best cell. Stage labels follow
    Figure~\ref{fig:manifesto-fg-ladder-heatmaps}; $f^1 g^0$ is the
    pre-alternation anchor and $f^1 g^1$ the first alternation
    step.}\label{tab:fg-ladder-perdim}
\end{table}

Table~\ref{tab:fg-ladder-perdim} places five of six dimensions in
the neighborhood of the $0.78$--$0.95$ band of Benoit's Table~3
split-expert references, with environment at the band's lower
edge; decentralization ($0.540$, against its own
split-expert reference of $0.78$) stays the hard case, as it is at
every chunk size of the fixed-prompt sweep. The stage column carries
the honest reading of the alternation: economic, social, and
European integration peak two or three steps past the anchor,
immigration and environment peak at the first step, and
decentralization peaks at the anchor itself, so prompt updates past
the first round buy improvements on three of the six dimensions.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{manifesto_singledim_per_dim_live.pdf}
    \caption{Single-dimension alternating ladder across all six
    Benoit dimensions. Each panel plots held-out (test) external
    Pearson $r$ against the Benoit expert mean by token leaf size
    ($256$--$8192$), for the evaluation after the first alternation
    round and at the best round per cell. Horizontal references per
    panel: the \citet{BenoitEtAl2025} proprietary-ensemble
    correlation, their best open-weight replication, and their
    Table~3 split-expert reference for that dimension. Environment
    cells at leaves $256$--$2048$ come from a re-evaluation pass on
    the same held-out split; all other cells come from the primary
    run.}\label{fig:fg-ladder-perdim-live}
\end{figure}

Figure~\ref{fig:fg-ladder-perdim-live} shows the per-dimension leaf
profiles: the leaf-size robustness of the economic lane recurs on
every dimension, with the per-leaf best inside a $0.056$-wide band
on economic, social, immigration, and European integration across
the $32\times$ leaf range, and decentralization flat and low
relative to its $0.78$ reference.

\paragraph{Joint shared-summarizer variant.} A joint variant trains
one shared summarizer for all six dimensions, with per-dimension
scorers, over the same token-leaf grid.
Figure~\ref{fig:fg-ladder-perdim-joint} plots its external Pearson
per dimension across leaf sizes and stages. The joint best per
dimension sits within $0.044$ of the single-dimension best on the
five dense dimensions. Decentralization is the cost of sharing: the
joint run tops out at $0.461$, below the $0.540$ the
single-dimension lane already reaches at its $f^1 g^0$ anchor, so
the shared summary crowds out the evidence the weakest dimension
needs.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{combined_perdim_leaf_invariance.pdf}
    \caption{Joint shared-summarizer ladder, per dimension. Each
    panel plots held-out external Pearson $r$ against the Benoit
    expert mean by token leaf size ($2^8$--$2^{13}$) for stages
    $f^1 g^0$, $f^1 g^1$, $f^2 g^1$, and $f^2 g^2$. Horizontal
    references per panel: the Table~3 split-expert value, the
    Benoit $18$-score ensemble, and the best Benoit open-weight
    replication. The five dense dimensions are leaf-stable near
    their references; decentralization sits below all three of its
    references at every leaf size and
    stage.}\label{fig:fg-ladder-perdim-joint}
\end{figure}

Figure~\ref{fig:fg-ladder-perdim-joint} makes the trade concrete:
sharing one summarizer preserves leaf-stability and reference-level
agreement on the five dense dimensions and gives up the weakest
dimension, so the single-dimension lanes are the configuration of
record for decentralization.

\paragraph{What remains.} A fixed-prompt baseline at matched
token-leaf sizes and the same prompt-search budget would let the
ladder numbers be compared directly against the chunk-invariance
figures of Table~\ref{tab:benoit-headline} rather than against the
scorer-only ablations block; that sweep has not been run.
