...
has a completed control arm (rate $0$: gold children only, whose
collapse signature is quoted above) and an unfinished treatment arm
(rate $1$): the treatment's $g$-stage evaluation did not complete.
\fixme{scheduled-sampling treatment arm (rate 1) g-stage evaluation
pending; table to be added on completion.}
...
task'' from ``the program learned the runtime.'' The runs exist but
their report stage produced no artifacts. \fixme{gold-to-DiffusionGemma
transfer reports empty; re-run report stage before citing.}
