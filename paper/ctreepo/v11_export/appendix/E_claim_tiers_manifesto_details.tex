...
The claim surface separates the evidence channels because they support
different claims.
...
\bottomrule
\end{tabular}
\caption{\textbf{Claim tiers.} The tiers can reinforce one another,
but they should not be collapsed.}
\label{tab:v8-claim-tiers}
\end{table}

The Benoit replication in the main text is root-observed: each held-out
...
The Benoit replication uses party election programs from the
Manifesto Project matched to expert-survey means released by
\citet{BenoitEtAl2025}. The
...
Benoit-replication table uses the reported character-leaf comparison. The
...
\(0.584\) at \(2\mathrm{K}\). The current interpretation is that finer leaves force
...
Dollar costs depend on hardware ownership, utilization, and API pricing. The
reproducible comparison is the call structure: one local open-weight model
with a tree state map versus a proprietary ensemble with multiple frontier
models and multiple prompt settings.
...
\path|assets/benoit/tables/manifesto_fg_ladder.tex| for provenance.
The headline results table reports the six-dimension comparison.
...
Main six-dimension Benoit-replication comparison,
...
Generated economic \(f/g\) stage grid retained for provenance. \\
\path|assets/benoit/tables/length_buckets.md| and
\path|assets/benoit/tables/era_breakdown.md| &
Robustness diagnostics summarized here. \\
\bottomrule
\end{tabular}
\caption{\textbf{Manifesto artifact provenance.} The generated files are fixed
artifacts for the reported result tables and figures.}
