% Derived from classical_parity_hll.tex (auto-generated); longtable form for page breaking. Do not edit rows by hand.
\begin{longtable}{llrrrrrrrrrrrrr}
\caption{HLL parity grid across the \texttt{(method, oracle, precision, $L$)} cells. \texttt{RSE} is the HLL theoretical relative standard error $1.04/\sqrt{2^p}$. \texttt{state$=$} and \texttt{bytes$=$} are per-cell equality rates for the classical rows; \texttt{rel MAE} is each method's relative mean absolute error against the oracle; \texttt{C1} and \texttt{C3} are the per-leaf and per-merge MAE; \texttt{wall$\times$} is the tree-to-flat wall-time ratio.}\label{tab:classical-parity-hll}\\
\toprule
