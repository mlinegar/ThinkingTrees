...
The paper arcs from the classical to the learned case along a single
...
a task whose oracle defeats any commutative sketch and a tree-plus-FNO
...
Section~\ref{sec:manifesto-llm} then takes the
...
representation. The paper's main finding, stated plainly, is that for
tasks whose structure aligns with the local laws, reliable measurement
on long documents becomes a question of audit budget rather than of
full-document annotation --- a trade that restricts the framework's
scope to locally-decomposable oracles but is useful in many
social-scientific settings where such decomposition is natural.
