...
The remaining results come in three tiers. The \emph{structural} tier
asks whether local validity composes to the root summary. The
...
The \emph{certificate} tier adds the transport and sampling assumptions
needed to turn sampled audits into a finite-sample bound.
...
Proposition~\ref{prop:mergeable-reduction}, stated in Section~\ref{sec:mergeable-sketches} as the paper's thesis, is the strengthening of this theorem to the deterministic global setting: when the local laws are replaced by global identities and $\fstar$ admits an oracle-level merge, $g$ becomes a classical mergeable summary. Section~\ref{sec:hll-parity} verifies this collapse numerically on HyperLogLog. The remainder of this section takes the learned version one step further --- from preservation at the root to equivalence of optimization objectives.
...
\]
where $R$ is the number of re-summarization rounds. So the
...
\noindent Exact preservation gives $\Delta_R = 0$ and therefore zero gap; approximate preservation gives a gap no larger than a method-specific Lipschitz constant times the expected distortion.
...
The method-specific Lipschitz constant takes a different form in each case. For DPO, $L = 2|\beta| L_{\mathrm{pol}}$, where $L_{\mathrm{pol}}$ is the policy Lipschitz constant. For GRPO-PL, $L = L_{\mathrm{grpo}}$ from the Plackett--Luce ranking structure. For GRPO-RL, $L = L_{\mathrm{grpo-rl}}$ combines the clipping and KL penalty constants.
