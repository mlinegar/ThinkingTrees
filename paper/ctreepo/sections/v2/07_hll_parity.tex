...
oracle that defeats any classical commutative sketch. This section
closes the loop in the opposite direction: when the oracle \emph{is} a
...
merge operator (register-wise max) is associative, commutative, and
idempotent --- exactly the ingredients of Proposition~\ref{prop:mergeable-reduction}.
...
This is Proposition~\ref{prop:mergeable-reduction}'s collapse made
concrete. Under A1--A3 the learned framework reduces to the classical
sketch; for a register-based implementation the reduction is literally
byte-for-byte.
...
\begin{table}[t]
    \centering
    \small
    \input{assets/hll/tables/classical_parity_hll.tex}
...
When the conditions of Proposition~\ref{prop:mergeable-reduction} hold,
