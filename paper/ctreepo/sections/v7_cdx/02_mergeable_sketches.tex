...
oracle. The same residuals are used twice: training data push \(g\)
toward local-law compatibility, and a runtime audit estimates the
remaining error on the realized states a tree actually produces. A
classical sketch gets correctness from algebra; a learned C-Tree gets
an auditable error bound.
...
Read the section as a ladder. Classical mergeable summaries provide
the template: summarize leaves into compact states, merge states, and
query only at the root. C-Trees keep that state-level shape but learn
the summarizer and query for tasks where no hand-built sketch exists.
Exact local laws recover the classical valid-summary guarantee; audited
approximate laws recover expected or certified error bounds. Neural
operators and Fourier neural operators enter only after that target is
clear: they are representation routes for learning a state machine that
...
This notation has one job: keep raw data, stored state, and query
answer separate. The classical sketch supplies all three by design;
the C-Tree will learn the stored-state layer and then audit whether the
query answer survives.
...
This is Assumption~\ref{ass:context} below, and it is a real restriction.
Tasks whose oracle is sensitive to discourse (irony, rhetorical tone, anything
where local substitution of equivalent content changes how the
surrounding text is read) are out of scope.

A classical sketch establishes correctness once, by the algebra of the
sketch, and never has to re-check at runtime. C-TreePO replaces that
algebraic guarantee with two complementary pressures on the same
residuals: training penalizes them, and a runtime audit measures them
on the realized states a deployed tree produces. Because \(g\) is
learned, the local laws below are not identities of the operator; they
are empirical obligations on the calls the tree actually makes. The audit samples
...
Section~\ref{sec:theorems} turns the sampled residuals into a
root-error bound. The laws also define the learnable target class:
...
this realized tree run. Section~\ref{sec:theorems} gives the
...
a runtime audit on the realized residuals. The rest of the section now
follows the same order a reader should use. First,
\S\ref{sec:v7-ctreepo-correspondence} shows that classical sketches
are not merely analogies: on the same tree, their validity and
mergeability obligations sit inside the C-Tree local laws. Next,
\S\ref{sec:v7-sketch-inventory} and
\S\ref{sec:v7-tree-comparisons} separate claim status from empirical
evidence: which sketch families come with classical guarantees, and
what the learned/tree reductions do in practice. Finally,
\S\ref{sec:v7-neural-operator-route} asks how a learned model can
approximate the local-law-compatible class when no hand-built sketch
exists.
...
gold-standard labels in the research design. We use the standard
survey-sampling machinery here rather than re-derive it: known
inclusion probabilities give Horvitz--Thompson/H\'ajek IPW estimates,
design-based standard errors, and effective-sample-size diagnostics
\citep{HorvitzThompson1952,Kish1965SurveySampling,SarndalEtAl1992,EgamiEtAl2024}.
In the tree audit, the finite population is the realized set of local-law
checks: leaf sufficiency checks, merge checks, and re-summary checks. The
sampled outcome is the corresponding local residual or violation indicator.
DSL supplies the estimator and uncertainty accounting; C-TreePO supplies the
local-law outcomes and interprets their aggregate error as a root-oracle
certificate.
...
Section~\ref{sec:theorems} compiles the residuals into the root-level
...
This subsection makes the nesting claim explicit in three steps. The
forward step runs a classical sketch on the same leaf partition and the
same binary tree, so its valid-leaf and valid-merge obligations become
C1 and C3. The reverse step reads exact C-Tree local laws as a
valid-summary guarantee at the root. The schedule step then explains
what changes when the same leaves are merged under another allowed
tree: C-TreePO gives query-level equivalence, while classical sketches
may additionally give byte-identical state equality.
...
following proposition states the resulting equivalence.
...
been merged, and A1, A2 give the C-Tree-side preservation.
Appendix~\ref{app:proof-mergeable} gives the detailed derivation.
...
The correspondence above tells us when a classical sketch instantiates
a C-Tree directly. The next move is classifying which concrete sketch
families we use, and in what sense. Some come with classical
mergeability guarantees that can support the local laws. Others are
empirical references only. A few are adjacent geometric background and
should not be attributed to the same source. Read
Table~\ref{tab:v7-sketch-inventory} as an evidence ledger: it says what
each family contributes to the C-Tree story before the experiments
start. The distinction matters for citation honesty:
\citet{Agarwal2013MergeableTODS} does not cover
\(\varepsilon\)-kernel geometry, so that row is adjacent background
unless supported by a separate source.
...
\toprule
Family & Description & Claim status & What the C-Tree uses \\
\midrule
HLL & Estimates the number of distinct items without storing the items
themselves; the state is a vector of registers. & classical guarantee + empirical & Register-state summary, pointwise-max merge, and final estimator query value; the tree and flat reductions agree when they use the same state representation. \\
Count-Min & Estimates item frequencies with hashed counter tables; the query
asks for the approximate count of one item. & classical guarantee + empirical & Additive table summary and point-frequency query value; mergeable but not idempotent. \\
Misra--Gries / SpaceSaving & Find heavy hitters: items whose frequencies are
large enough that they should survive compression. & classical interfaces & Capacity, mass, and error-envelope arguments for Misra--Gries; SpaceSaving follows by the standard min-counter correspondence. \\
GK quantiles & Maintains a compressed ordered summary so approximate ranks and
quantiles can be answered from a stream. & classical one-way interface & Sequential/incremental quantile interface; not claimed as arbitrary full state-state mergeability. \\
KLL quantiles & Randomized quantile sketch that compacts weighted samples and is
designed for mergeable quantile states. & randomized classical interface & Randomized sized mergeable query-sketch interface with a weighted-mass invariant. \\
Interval/range constructions & Keep small subsets or colored states that approximate many interval or range counts at once. & classical scaffolds + conditional size-reduction constructions & Same-weight interval concentration, finite VC/Sauer--Shelah trace layer, and low-discrepancy coloring route when supplied by a certificate. \\
Markov and bigram sketches & Store the count information needed to answer simple sequence-model queries, such as transition or bigram statistics. & local-law examples & Task-specific answer-sufficient states showing the same summarize/merge/query pattern outside classical data-stream queries. \\
CPC, Theta/KMV, Frequent Items, classic quantiles, REQ, t-digest, Tuple, VarOpt & Additional library sketch families used to compare tree reductions against familiar implementations. & empirical-only here & Empirical comparisons only; no classical guarantee is claimed for these additional variants. \\
...
The empirical comparisons now use the correspondence rather than
restate it. HLL supplies the exact state-level parity check: when the
tree uses the native register state and merge, it should collapse to
the flat sketch. Learned HLL-like states test how hard it is to recover
a supplied state/query pair. Broader sketch families then show the
same pattern for frequency, quantile, set, and sampling-style tasks:
some targets are recoverable by the learned class, while others leave a
structural gap. The classical anchor is HyperLogLog
...
rows; Appendix~\ref{app:sketch-simulations} records the full generated
grid, per-family panels, and run-artifact provenance.
...
references; claim status is the separate classification in
...
The sketch experiments validate the bridge in two directions: exact
state-level collapse when the classical state is supplied, and learned
recovery or structural error when the state must be fit from oracle
feedback. That leaves the representation question. When no hand-built
sketch is available, neural operators are one candidate route because
they can learn maps between structured states, including the leaf and
merge maps a C-Tree needs.
...
The guarantee below is the learned analogue of the classical nesting
claim: if an exact local-law summarizer exists in the class, and the
learned operator is close to it on the leaf, merge, and re-entry calls
the tree actually makes, then the root error is bounded.
...
approximation bound supplies the same realized-call guarantees. If the
...
the local-law accounting converts that budget into
expected-\(\varepsilon\) validity.
...
This is also where sample size enters the paper. More labels do not
automatically make the learned residual smaller; they make the audit
estimate sharper under the usual design-based conditions. The next box
therefore reads the local-law certificate as a standard finite-population
estimand, IPW estimate, standard error, calibration margin, and
oracle-transfer slack.

\begin{center}
\fbox{\parbox{\dimexpr0.96\linewidth-2\fboxsep-2\fboxrule\relax}{
\textbf{Econometric reading.}
Condition on the realized corpus, fitted tree pipeline, and logged audit
design. The finite population is the set of local-law checks on the realized
trees. The outcome is the C1/C3/C2 residual or violation indicator. The
estimand is the aggregate local-law error \(E_{\mathrm{law},R}\). The audit
reports its HT/H\'ajek/IPW estimate
\(\widehat E_{\mathrm{law},R}^{\mathrm{IPW}}\) and design-based uncertainty
\(c_\alpha\widehat{\mathrm{SE}}_{\mathrm{IPW}}\). Calibration contributes
\(B_{\mathrm{cal}}\), and transferring from \(\widehat f\) to \(\fstar\)
contributes \(2\varepsilon_{\mathrm{orc}}\). Thus the econometric certificate
is the audited local-law estimate plus sampling uncertainty, calibration
margin, and oracle-transfer slack.
}}
\end{center}

A compact way to state the learned guarantee is to instantiate this standard
DSL/IPW certificate on the local-law outcomes rather than introduce a new
residual term. Write
\(\widehat E_{\mathrm{law},R}^{\mathrm{IPW}}\) for the HT/H\'ajek
C1/C3/C2 estimate and \(\widehat{\mathrm{SE}}_{\mathrm{IPW}}\) for its
design-based standard error, both computed from the logged node sample.
\(B_{\mathrm{cal}}\) is the separate held-out query/oracle calibration margin,
and \(c_\alpha\) is the confidence multiplier. If \(\widehat f\)
uniformly recovers \(\fstar\) within \(\varepsilon_{\mathrm{orc}}\), the
practical certificate is
\[
\begin{aligned}
  \mathbb E_{s\sim Z_R(g_\theta,x,T)}
  d_Y(\fstar(s),\fstar(x))
  &\le
  ...
  +
  \underbrace{c_\alpha\,\widehat{\mathrm{SE}}_{\mathrm{IPW}}}
...
through the calibrated query \(\widehat f\). The law-of-large-numbers and
effective-sample-size behavior belongs to the cited DSL/survey-sampling layer:
larger design information shrinks the standard-error term under the usual
conditions, while the learned local-law residual remains an empirical model
quantity. The sample-size shorthand is therefore the envelope condition:
if \(c_\alpha\widehat{\mathrm{SE}}_{\mathrm{IPW}}\le
C/\sqrt{n_{\mathrm{eff}}+1}\), then the certified error is bounded by the same
display with that envelope replacing the standard-error term. A
neural-operator architecture is covered when the realized state machine lies
in, or is certified near, the local-law-compatible class.
...
The preceding discussion leaves a simple picture:
\begin{enumerate}[leftmargin=1.5em]
\item Classical mergeable summaries and C-Trees have the same operational
shape: summarize leaves into states, merge states, and query the root.
\item The correspondence is state-level. Scalar child answers do not have
to merge; the state must carry whatever the root query will need.
\item Exact local laws make the realized root state query-equivalent to
the represented input. Randomized summaries keep their original success
probability, and approximate laws give expected or certified error bounds.
\item Neural operators supply candidate leaf and merge maps when no
hand-designed sketch is available. The audit checks whether the learned
maps behave like a mergeable summary on the deployed run.
\item The empirical suite checks both sides: exact collapse to classical
HLL when the state is fixed, and learned recovery or structural failure
when the state is trained from oracle feedback.
\end{enumerate}
...
Section~\ref{sec:min-manifesto-results}. The empirical sequence follows
the same division. The sketch simulations in this section are compact
validation anchors for the framework. The Manifesto results are the
full semantic LLM application, where the root score is externally
validated and the available local diagnostics show where future
node-level audit labels would enter.
