\subsection{Gold Labels at Every Node: Oracle-Grounded Training}\label{sec:manifesto-qsentence}\label{sec:rile-oracle}

The pipeline above is prompt-only: the oracle enters only at the root,
through expert-survey means, and the local laws appear only as audit
probes and prompt-design pressure. The Manifesto Corpus supports a
stronger design. Trained human coders first split each platform into
quasi-sentences (clause-level units; roughly one policy statement per
unit) and then assign each coded quasi-sentence one category from the
Comparative Manifesto Project (CMP) scheme. In our release slice, those
annotations cover $2{,}157$ platforms ($\sim\!2.27$M coded spans). The
hand labels stop there: no coder labels a C-Tree merge. But because the
codes aggregate additively over contiguous spans, the quasi-sentence
labels induce a gold oracle value at \emph{every node of every tree}:
each leaf, each internal merge span, and the root. The oracle-grounded
local-law supervision path that works in the controlled settings
therefore transfers to this corpus with no oracle redesign; the needed
span targets are induced from existing annotations. This is the
known-oracle version of the question the alternating prompt ladder
raises empirically: when local targets are available, train against
them directly; when only prompt programs are being updated, measure
whether the co-adapted pair still behaves like a local operator. Below
are the two label systems, the harness that runs the experiments, and
the results, including the leaf-size ladder
(Appendix~\ref{app:qsentence-leaf-ladder}) and gold-seeded
summarization (Appendix~\ref{app:gold-seeding}).

\subsubsection{Two Label Systems: Documents and Quasi-Sentences}\label{sec:qsentence-labels}
...
\paragraph{Document-level labels.} Each platform carries document-level
targets. The first is the pair used in the document-label experiments
above: the \citet{BenoitEtAl2025} expert-survey means on six seven-point
policy scales, available for the $235$-platform benchmark. The second is
the Manifesto Project's own published MPDS row
\citep{ManifestoProject2025a}: one electoral program per row, with total
coded quasi-sentence counts and per-category shares. For every annotated
platform used below, the Corpus quasi-sentence rows can be linked back
to this document row. The document-level MPDS targets include the RILE
left--right index on $[-100, 100]$ and per-domain salience shares. RILE
is a deterministic function of the CMP category counts: with $R$ the
count of quasi-sentences bearing one of $13$ designated right codes, $L$
the count bearing one of $13$ left codes \citep{LaverBudge1992}, and
$N$ the total count of coded non-header quasi-sentences,
...
\citep{ManifestoCorpus2025} provides the underlying
per-quasi-sentence annotations: each coded quasi-sentence carries one
CMP category code from a $56$-category scheme, assigned by a trained
human coder under the project's handbook. These are the hand-coded gold
labels. For any contiguous span $x$ of a platform, aggregating the
codes of the quasi-sentences inside $x$ and applying
Equation~\eqref{eq:rile} yields a gold oracle value $\fstar(x)$: an
$8$-vector
...
span is a known function of its children's counts. Thus the tree does
not need human labels on merges. The two children and the parent inherit
targets from the same quasi-sentence table, making the C3 triangle
observable on real political text. Every node of every C-Tree over a
coded platform therefore has a gold target, which is the structural
property the controlled settings relied on and that generic
long-document corpora lack.
...
\subsubsection{Bundles, Splits, and the \texttt{treepo} Harness}\label{sec:qsentence-harness}
...
involved in labeling, and no merge annotation is collected by hand; the
parent target is the count rollup of the quasi-sentences under that
parent. The corpus is $218$ Western European platforms
...
Section~\ref{sec:rile-fg-ladder} for any \emph{family}: an object
supplying its own parameterization of the summarizer $g$ and readout
$f$ behind a fixed four-method contract, while the surrounding loop
(alternation schedule, node sampling, metrics, audit logging) stays
family-agnostic. The contract is the operator interface of
Section~\ref{sec:framework} made executable;
Appendix~\ref{app:treepo-software} documents the package, the contract,
a minimal three-call vignette, and the CLI that ran the sweeps below.
Two families instantiate the contract here, chosen
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Neural-operator family} (\texttt{fno}): leaves are
          ...
          losses, exactly as in Appendix~\ref{sec:fno-primer}. The state
          is a learned vector.
\end{itemize}
Both families consume the same bundles and the same splits, and every
run leaves behind the same artifact schema (per-stage grid summaries,
per-node prediction records, iteration histories), so their numbers
are directly comparable: the substrate comparisons in
Appendix~\ref{app:qsentence-leaf-ladder} are a table join rather than
...
\subsection{Trained Substrates: The Learned Merge Composes Where the Signal Is Dense}\label{sec:qsentence-results}
...
fixed-prompt pipeline of Section~\ref{sec:benoit-headline} did not have:
...
while the headline reward looks healthy. It is also the same failure
the joint prompt ladder hit on decentralization
(Section~\ref{sec:rile-fg-ladder}), reproduced on a trained substrate
with gold supervision.
...
Section~\ref{sec:benoit-headline} sits near $0.86$ on the same task, so
...
\subsection{What the Corpus Teaches the Framework}\label{sec:manifesto-lessons}

The two supervision levels answer different halves of one question,
and their answers compose into three lessons.

First, the local laws must be priced in the objective, or optimizers
will spend them. The two clearest failures in the corpus are the same
failure at different supervision levels. In the joint prompt ladder,
a mean-over-dimensions reward let the scorer pull the shared
summarizer away from the one dimension whose signal cost the average
(Section~\ref{sec:rile-fg-ladder}); in the trained-substrate
comparison, a mean-over-dimensions reward let the prompt-program merge
collapse to predicting corpus means while its headline reward looked
healthy (Section~\ref{sec:qsentence-results}). Both are the projection
story of Section~\ref{sec:theorems} running in reverse: an objective
whose zero set is larger than the local-law subspace lets the learned
operator drift off it. The remedies are correspondingly structural,
either reweight the objective per dimension (the two-track recipe) or
put the law terms into the reward, and the audit is what detects the
drift when neither has been done.

Second, signal density governs what a trained merge can learn. The
neural-operator family composes lawfully on every dimension whose gold
share crosses roughly $0.13$ and fails on every dimension below it,
with the failure taking the honest form of a zero predictor rather
than an inflated echo. Where the prompt-only pipeline's
chunk-invariance shows a fixed, competent summarizer moving freely
across leaf sizes, the trained substrate pays for its supervision:
its accuracy peaks at an intermediate granularity where labeled nodes
are plentiful and merge chains are still short.

Third, construct ceilings bound any decomposition before any model is
trained. The CMP codes determine the published document scores to
numerical precision (rollup $r = 0.9975$) and determine the
expert-survey constructs only partially (CV $R^2$ from $0.03$ to
$0.47$), and the trained substrates inherit exactly that ordering:
strong reconstruction of the internal target, external accuracy that
falls off dimension by dimension as the ceiling drops. A researcher
adopting this design gets the diagnosis before the compute: the label
system itself predicts which constructs local supervision can reach.

The prompt-only results show a tree of local calls preserving an
expert measurement target at parity with a frontier ensemble; the
oracle-grounded results show the same machinery training honestly
where the label system supports it and failing legibly where it does
not. Corpora without a gold node-level layer get the audit of
