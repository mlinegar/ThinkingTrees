...
follows is one of measurement design as well as prediction. If the
...
adds an earlier failure mode: the surrogate is both a compressed
representation of the original document and a predicted label.
...
We test the framework by replicating
\citet{BenoitEtAl2025}. The Comparative Manifestos Project (CMP) has
collected and hand-coded party election platforms for decades; its
expert-survey arm produces mean placements of each platform on
...
\(235\) party platforms (hereafter, the \emph{Benoit replication}).
...
synthetic audit designs to show how logged-propensity correction keeps
the node-level estimand centered while variance grows in the expected
places.

\paragraph{Map of sections and appendices.}
...
Section~\ref{sec:v10-controlled-checks} builds intuition from the
controlled Markov and IPW checks.
...
node-audit design.
Appendix~\ref{app:v10-markov} gives the controlled Markov mechanism.
Appendix~\ref{app:v10-ipw-stress} gives the IPW audit stress check.
