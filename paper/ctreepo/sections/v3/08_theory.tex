...
ends of the classical-to-learned gradient: one oracle requires ordered
boundary state (which unordered register-wise merges cannot carry),
while the other \emph{is} a classical sketch. We now state the theorems that govern
both, plus the setting yet to come
...
Before transporting $\Delta_R$ through a bound, we name where it
comes from. We do \emph{not} identify $\Delta_R$ directly with a
Banach-space approximation norm. Instead we use
