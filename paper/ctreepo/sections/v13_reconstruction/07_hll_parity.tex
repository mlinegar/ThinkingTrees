...
state. The converse test is a setting where the oracle \emph{is} a
classical sketch's query: does the C-TreePO framework recover the
classical sketch exactly? It does, up to byte-identity for a
register-based implementation, and the register-space constraint is
...
at each node, and classical HLL's register-wise structure makes C2
...
Appendix~\ref{app:classical-parity}.
...
summary up to oracle equivalence. The per-cell grid
(Table~\ref{tab:classical-parity-hll} in
Appendix~\ref{app:classical-parity}) confirms this on every cell:
...
C-TreePO reduces to the classical mergeable sketch byte-for-byte on the
reference implementation, not merely in distribution. This is
the tightest empirical form of the generalization claim
for this representative sketch; the formal version is the
...
classical sketch can; the HLL parity result shows it recovers the classical
