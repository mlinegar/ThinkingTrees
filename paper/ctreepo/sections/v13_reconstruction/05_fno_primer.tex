...
$g$ has to be a position-aware operator. One practical route to such a
$g$ (and, in the Markov experiments, a readout $f$ baked into its
training head) is a Fourier neural operator (FNO) backed by the
equation-(6) neural-operator architecture of
\citet{Kovachki23NeuralOperator}. Neural operators give
us a parametric class with explicit approximation guarantees on compact
realized calls and a clean bridge to the local-law budgets
(Proposition~\ref{prop:neural-operator-bridge}), which is why we use
them in the controlled benchmark. Other routes work too:
Section~\ref{sec:manifesto-llm} instantiates $f$ and $g$ as
...
particular parameterization. The contribution here is the
approximation-theoretic interface: an operator family that can be
applied across the realized leaf, merge, and re-summary calls of one
tree, with transfer moduli converting its tolerance $\varepsilon$ into
the budgets $(\varepsilon_{\mathrm{leaf}}, \varepsilon_{\mathrm{merge}},
...
One shift from Kovachki's setting: our ``domain'' is the index set of
the realized call currently being processed, not a PDE mesh alone.
...
reusable requirement on the operator family: a single
...
map $\phi$ by an equation-(6) neural operator. We use only the
...
budgets. The approximate local laws are phrased as audited violation
probabilities, so uniform operator approximation alone leaves them
unverified. The bridge of
...
The theorems below are agnostic to the Fourier structure: any learned
operator family satisfying the same realized-call approximation
interface and the same audited transfer conditions can replace it
unchanged.
...
finite encoder stack is an equation-(6) neural operator. A
transformer summarizer can therefore be placed in the same operator-learning
