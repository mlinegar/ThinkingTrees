...
The theorem stack is conditional. It gives preservation, schedule invariance,
and preference-objective transport for applications whose chosen state,
oracle, local laws, and sampling assumptions hold. These are also the
assumptions that make node supervision transportable to root objectives:
sampled leaves and merges are useful because they estimate preservation of
the same oracle that the root label anchors.
...
Leaf-size robustness is therefore a conditional claim. Chunking and merge
schedules are interchangeable inside the class where the realized local-law
checks hold. The node/root supervision tradeoff is an assumption-backed design
choice: local labels can substitute for root labels when they measure
preservation of the same oracle.
...
The certificate is the formal expert-attention allocation statement. Gold or
trusted root labels calibrate the target through $B_{\mathrm{cal}}$; sampled
node labels estimate the compression residual through $\hat\Delta_R$ and
$B_{\mathrm{est}}$; clipping records the price of stabilizing the design
weights.

Local-law weights in an optimizer should be read through this stack, not as a
new theorem. A weight such as $\lambda$ trades oracle fidelity against
sufficiency residual. Schematically, an objective of the form
$L_{\mathrm{oracle}}(f,\fstar)+\lambda L_{\mathrm{laws}}(f,g)$ asks the learned
readout and summarizer to stay close to the trusted oracle while also making
$g$ an approximate sufficient statistic for $f$ along the merge route. At
$\lambda=0$, the scorer can fit the oracle while the compression path remains
uncertified. At large $\lambda$, the learned pair is pulled toward the
local-law subspace, possibly at the cost of task fidelity if the state is too
narrow. This is the same tradeoff shape as an information bottleneck
\citep{TishbyPereiraBialek1999}, with the extra constraint that the
representation must compose through the tree. The audit reports the realized
residual after that design choice.
