...
that surface and re-exports only the bridge names used here.
...
This is the pattern C-TreePO generalizes: a bounded-size
...
generalization. The formal claim extends beyond HLL: any sketch that supplies the
...
Most interesting oracles lie beyond register-like queries. In the Markov example of Section~\ref{sec:markov-interlude}, the oracle counts boundary transitions between adjacent colored regions---a cross-span interaction that no commutative register-wise merge can recover from leaves alone. The merge operator has to carry boundary information forward, and that is what motivates the Fourier neural operator in Section~\ref{sec:fno-primer}. Section~\ref{sec:framework} develops the local laws, Section~\ref{sec:estimation} the audit, and Section~\ref{sec:theorems} the preference-learning consequences that make the learned version as trustworthy as the classical one.
