...
The theorems of Section~\ref{sec:theorems} are stated over an abstract oracle $\fstar$; they apply whether $\fstar$ is a distinct-count query (Section~\ref{sec:hll-parity}), a color-transition count (Section~\ref{sec:markov-walkthrough}), or a rubric-driven semantic score. Section~\ref{sec:social-measurement} fixed the measurement interpretation of the last case; here we implement it on Manifesto-Project RILE scoring and close the classical-to-learned arc. The summarizer $g$ is a prompted language model, the merge is an LLM-friendly prompt, and the readout $f$ is a second LLM call under a scoring rubric that maps the root summary to a seven-point scale. The oracle $\fstar$ is the rubric social scientists actually use, and the target benchmark is the recent comparative-politics paper \citet{BenoitEtAl2025}, which establishes the current empirical bar for LLM-based political-text measurement.
...
audit design, not over the weights of $g$ or $f$.

Three things remain shared across the two pipelines. First, the
...
propensities; the estimator is indifferent to whether the weights behind
...
annotations are already there. Section~\ref{sec:manifesto-qsentence}
develops this in full: the label construction, the \texttt{treepo}
implementation, and the trained-substrate results across leaf sizes
and policy dimensions. This is the known-oracle version of the same
question the alternating prompt ladder raises empirically: when local
targets are available, train against them directly; when only prompt
programs are being updated, measure whether the co-adapted pair still
behaves like a local operator.
...
manifesto measurement on a single open-weight model. The per-dimension
agreement ranks with the proprietary ensemble, and the chunk-invariance
pattern tracks the theory's projection story: when local-law error is
small, changing the chunk schedule moves the realized operator within the
same theorem-eligible region and leaves the target fixed. The compute
story is that mergeability pays here: once local laws hold, the tree
