...
We cannot afford to query the oracle at every node, so we sample. This is a finite-population survey problem: the audit units form a population, we draw a random subset, and we want an unbiased estimate of the population mean. The standard tool is the Horvitz--Thompson estimator, which reweights each sampled unit by the inverse of its inclusion probability. For queried unit $i$ with inclusion indicator $Z_i$, propensity $\pi_i$, and contribution $Y_i$:
\[
