...
This appendix formalizes the connection between our Oracle-Preserving Summarization (OPS) framework and the theory of \emph{mergeable summaries} developed for streaming and distributed databases \citep{Agarwal2013MergeableTODS}. We separate attribution cleanly: (i) closure/compositional correctness over merge trees is inherited from the mergeable-summary lineage, and (ii) oracle-semantic preservation plus objective/IPW certificate transport are TreePO-specific. In the deterministic commutative-oracle regime, Parentwise Compatibility recovers the classical mergeability condition; outside that regime (order-sensitive tasks, stochastic summarization), OPS extends the interface to free-monoid text composition.
...
\subsection{Classical-Sketch Sanity Check}

The new \texttt{treepo} package includes an explicit commutative sanity-check suite built around distinct counting. We use exact HyperLogLog as the classical mergeable baseline, stream items into leaf sketches token by token, and then merge those sketches under different tree schedules. This gives a clean reference point with three visible regimes: exact HLL should be schedule invariant, a learned TreePO sketch should approach the HLL floor as local supervision increases, and a deliberately wrong baseline such as summed leaf-unique counts should fail because it ignores cross-leaf overlap.
...
\begin{figure}[t]
    ...
    \small
    \textbf{Exact HLL.}
    Stream each item into a leaf sketch, then merge by elementwise max. The root estimate is invariant across balanced and left/right schedules, so any spread is a bug rather than a modeling choice.
    \end{minipage}}
    \hfill
    \fbox{%
    \begin{minipage}[t]{0.30\linewidth}
    \small
    \textbf{Learned TreePO sketch.}
    Train a learned mergeable state from local/root cardinality supervision. The main metric is distance to the HLL relative-RMSE floor at matched memory.
    \end{minipage}}
    \hfill
    \fbox{%
    \begin{minipage}[t]{0.30\linewidth}
    \small
    \textbf{Wrong baseline.}
    Summing leaf-level unique counts ignores overlap across leaves. It is therefore not merge-safe for distinct counting and provides a concrete failure case next to the classical sketch.
    \end{minipage}}
    \caption{Classical-sketch sanity check shipped in the \texttt{treepo} package: exact HLL streaming invariance, TreePO approach to the HLL floor, and negative-control failure.}
    \label{fig:treepo-cardinality-sanity}
\end{figure}
...
\paragraph{Certificate transport to TreePO objectives.}
The TreePO transport step is purely algebraic: if a method-specific certificate has the form
$|g| \le C\Delta$ and the summarization/audit procedure yields an upper bound $\Delta \le U$
(deterministically or on a high-probability event), then $|g| \le CU$ by substitution.
In our setting, $g$ is the method gap (DPO/GRPO) and $\Delta$ is the oracle distortion.
