...
The global variant proofs reveal an elegant algebraic structure: concatenation on strings induces an associative operation on oracle values.

\paragraph{The Key Insight: Quotient Structure.}
The task metric $\metric$ on $\Y$ defines an equivalence relation: $y \sim y'$ iff $\metric(y, y') = 0$. The proofs show that:
\begin{enumerate}
    \item Concatenation respects this equivalence (Proposition~\ref{prop:congruence-strings})
    \item The induced operation on equivalence classes is associative (Proposition~\ref{prop:odot})
\end{enumerate}

\paragraph{Proof Technique: Route Comparison.}
Both propositions use the same technique: compare two ``routes'' from a source to a target:
\begin{itemize}
    \item \textbf{Direct route:} Compute $\fstar(a \concat b)$ directly
    \item \textbf{Summary route:} Compute $\fstar(g(g(a) \concat g(b)))$ via summaries
\end{itemize}
The global two-route compatibility (Assumption~\ref{ass:app2}) guarantees these routes arrive at the same oracle value.

\paragraph{Triangle Inequality Chaining.}
When comparing two oracle values that arise via different paths, we chain triangle inequalities through intermediate values that are ``known'' to be equal (by the global assumptions).
