...
from treepo_cdx._json import jsonable, stable_digest


OBJECTIVE_SCHEMA_VERSION = "treepo_cdx.objective.v1"
...
_LEGACY_PUBLIC_FIELDS = {"gap_weight", "oracle_gap_weight", "lambda_eff", "reliability"}
...
class ObjectiveSpec:
    ...
    def _normalized_terms(self) -> dict[str, dict[str, Any]]:
        ...
        for raw_name, raw_payload in dict(self.terms or {}).items():
            ...
            if name in _LEGACY_TERM_NAMES:
                raise ValueError(f"objective term {raw_name!r} is evidence metadata, not a training term")
            if name not in {OBJECTIVE_TERM_ROOT, OBJECTIVE_TERM_LOCAL_LAW_CORRECTED}:
                raise ValueError(f"objective term {raw_name!r} is not public")
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "local_law_estimator": self.local_law_estimator,
    ...
    def from_mapping(cls, payload: Mapping[str, Any]) -> "ObjectiveSpec":
        ...
        if legacy_fields:
            raise ValueError("legacy public objective fields are not supported: " + ", ".join(legacy_fields))
