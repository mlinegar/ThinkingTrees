#!/usr/bin/env python3
"""Run Markov changepoint boundary-detection honesty simulation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Markov changepoint honesty simulation."
    )
    ...
    parser.add_argument("--min-leaf-tokens", type=int, default=1)
    parser.add_argument("--max-leaf-tokens", type=int, default=8)
    parser.add_argument("--fixed-leaf-tokens", type=int, default=2)
    ...
    parser.add_argument(
        "--json-summary",
        type=str,
        default="outputs/markov_changepoint_honesty_summary.json",
        help="JSON summary output path.",
    )
    parser.add_argument(
        "--csv-summary",
        type=str,
        default="outputs/markov_changepoint_honesty_summary.csv",
...
def main() -> int:
    ...
    summary = run_markov_changepoint_honesty_experiment(
        config,
    ...
    print(
        "boundary_f1 | fixed={:.6f} | honest={:.6f} | leaky={:.6f}".format(
            fixed.boundary_f1,
            honest.boundary_f1,
            leaky.boundary_f1,
        )
    ...
    print(
        "boundary_cost | fixed={:.6f} | honest={:.6f} | leaky={:.6f}".format(
            fixed.mean_boundary_cost,
            honest.mean_boundary_cost,
            leaky.mean_boundary_cost,
        )
    )
