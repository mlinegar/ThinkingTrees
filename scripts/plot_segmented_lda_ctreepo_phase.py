...
def main() -> int:
    ...
    x_vals = sorted({float(r["eval_internal_query_rate"]) for r in rows if np.isfinite(float(r["eval_internal_query_rate"]))})
    y_vals = sorted({float(r["calibration_leaf_query_rate"]) for r in rows if np.isfinite(float(r["calibration_leaf_query_rate"]))})
