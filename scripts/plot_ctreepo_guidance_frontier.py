...
def _extract_row(payload: dict) -> dict:
    ...
    cost_ratio = budget_q / oracle_q if np.isfinite(budget_q) and np.isfinite(oracle_q) and oracle_q > 0 else float("nan")
    return {
...
def main() -> int:
    ...
    rows = [_extract_row(json.loads(Path(p).read_text(encoding="utf-8"))) for p in glob.glob(str(args.input_glob), recursive=True)]
    ...
    if cal_targets:
        rows = [r for r in rows if _float_in_set(float(r.get("calibration_leaf_query_rate", float("nan"))), cal_targets)]
