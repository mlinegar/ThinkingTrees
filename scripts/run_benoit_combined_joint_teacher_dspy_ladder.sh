#!/usr/bin/env bash
# Build all-six-dimension Manifesto teacher traces with one joint scorer call
# per tree node, then run the joint DSPy f/g alternating ladder.
...
SCORE_MAX_TOKENS="${SCORE_MAX_TOKENS:-700}"
SCORE_MAX_CHARS="${SCORE_MAX_CHARS:-50000}"
...
echo "=== $(date -u) :: joint-teacher all-six Manifesto ladder root=${ROOT} ==="
...
if [[ "${SKIP_TEACHER:-0}" == "1" ]] || [[ "${leaf_traces_exist}" == "1" ]]; then
  echo "=== $(date -u) :: reusing joint teacher traces in ${TEACHER_DIR} ==="
else
  echo "=== $(date -u) :: scoring all six dimensions in one pass source_kind=${SOURCE_KIND} ==="
  ./venv/bin/python scripts/run_manifesto_teacher_fg_joint_leaf_grid.py \
    ...
    --score-max-tokens "${SCORE_MAX_TOKENS}" \
    --score-max-chars "${SCORE_MAX_CHARS}" \
  --output-dir "${TEACHER_DIR}" \
  2>&1 | tee "${ROOT}/teacher_joint.log"
fi
...
./venv/bin/python scripts/plot_manifesto_fg_ladder_grid.py \
  ...
  --figure-subtitle "Macro average across economic, social, immigration, EU, environment, and decentralization; one-pass joint teacher f* labels." \
  --output-dir "${PLOT_DIR}" \
