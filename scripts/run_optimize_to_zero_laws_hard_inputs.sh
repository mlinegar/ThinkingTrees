#!/usr/bin/env bash
# Hard-input laws experiment: can learned_local_laws drive sufficiency from
# raw-ish token inputs (where the regime structure is NOT directly exposed)?
# leaf=2 (C2 merge active) and leaf=4. Encodings: regime_ids, one_hot_token_ids, normalized_token_ids.
set -euo pipefail
source venv/bin/activate

BUNDLE=outputs/_bundles/markov_hazard_panels/paper_hazard_panel_v1_t128/seed_0/base_bundle.json
OUT_ROOT=outputs/optimize_to_zero_laws_hard_inputs
mkdir -p "$OUT_ROOT"

GPU="${1:-3}"

for encoding in regime_ids one_hot_token_ids normalized_token_ids; do
  for leaf in 2 4; do
...
rows = []
root = "$OUT_ROOT"
for enc in ("regime_ids", "one_hot_token_ids", "normalized_token_ids"):
    for leaf in (2, 4):
