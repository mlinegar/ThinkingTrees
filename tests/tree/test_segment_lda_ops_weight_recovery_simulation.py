...
def test_topic_estimator_metadata_records_realized_device_policy():
    ...
    assert summary.topic_meta["device_requested"] == "cpu"
    assert summary.topic_meta["device_used"] == "cpu"
    assert summary.topic_meta["device_mode_resolved"] == "cpu"
...
def test_neural_hybrid_topic_estimator_runs_end_to_end():
    ...
    assert np.isfinite(float(summary.topic_meta.get("topic_phi_l2_error_mean", float("nan"))))
    assert summary.topic_meta.get("topic_phi_neural_mergeable_selection_mode") == "final_step_no_validation"
    assert summary.topic_meta.get("topic_phi_neural_mergeable_selection_split") == "config"
    assert summary.topic_meta.get("topic_phi_neural_mergeable_selection_metric_name") == "train_loss"
