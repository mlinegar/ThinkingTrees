...
# =============================================================================
# Unified RILE Metric Factory (Phase 5)
# =============================================================================

def create_rile_metric(
    ...
    """
    Create a DSPy-compatible metric from any RILE scorer.

    This is the unified factory for all RILE scoring scenarios, supporting:
    - Similarity: Compare two texts using scorer.score(text_a, text_b)
    - Prediction: Score single text, compare to ground_truth_rile
    - Ground truth: Compare two RILE scores directly (no text scoring)

    Args:
        scorer: The scorer to use. Can be:
            - RILESimilarityScorer (implements ScoringOracle)
            - RILEOracleClassifier (has predict_rile method)
            - Callable returning RILE score
            - None (for ground_truth mode)
        mode: Scoring mode - "auto", "similarity", "prediction", "ground_truth"
        ground_truth_field: Field name for ground truth RILE in gold example
        prediction_field: Field name for text to score in prediction
        threshold: RILE points for threshold-based metrics (not currently used)
        max_error: Scale for error normalization. Default RILE_RANGE (200).
            - RILE_RANGE (200): Full scale. 100-point error = 0.5 score.
            - Smaller values are more lenient (e.g., 100 means 100-point error = 0 score).
        with_feedback: If True, return {'score': float, 'feedback': str} for GEPA

    Returns:
        DSPy-compatible metric function

    Examples:
        # From similarity scorer (auditing)
        metric = create_rile_metric(RILESimilarityScorer())

        # From prediction classifier (summarization training)
        metric = create_rile_metric(oracle_classifier, mode="prediction")

        # From ground truth comparison (evaluation)
        metric = create_rile_metric(None, mode="ground_truth")

        # Custom scale (more lenient)
        metric = create_rile_metric(None, mode="ground_truth", max_error=100.0)
    """
    # Auto-detect mode based on scorer type
    if mode == "auto":
        mode = _detect_rile_mode(scorer)

    if mode == "similarity":
        return _create_rile_similarity_metric(scorer, prediction_field, with_feedback)
    elif mode == "prediction":
        return _create_rile_prediction_metric(scorer, ground_truth_field, prediction_field, max_error, with_feedback)
    elif mode == "ground_truth":
        return _create_rile_ground_truth_metric(ground_truth_field, max_error, with_feedback)
    else:
        raise ValueError(f"Unknown mode: {mode}. Use 'similarity', 'prediction', or 'ground_truth'")


def _detect_rile_mode(scorer: Optional[Any]) -> str:
    """Auto-detect the appropriate mode based on scorer type."""
    if scorer is None:
        return "ground_truth"
    if hasattr(scorer, 'score') and callable(getattr(scorer, 'score', None)):
        return "similarity"  # ScoringOracle protocol
    if hasattr(scorer, 'predict_rile') and callable(getattr(scorer, 'predict_rile', None)):
        return "prediction"  # RILEOracleClassifier
    if callable(scorer):
        return "prediction"  # Generic callable
    raise ValueError(f"Cannot auto-detect mode for {type(scorer)}")


def _create_rile_similarity_metric(
    scorer: Any,
    prediction_field: str,
    with_feedback: bool,
) -> callable:
    """Create metric for similarity mode (comparing two texts)."""
    from .position_oracle import RILESimilarityScorer

    # Auto-create scorer if not provided
    if scorer is None:
        scorer = RILESimilarityScorer()

    def metric(gold, pred, trace=None, pred_name=None, pred_trace=None):
        # Extract texts
        original = getattr(gold, 'original_content', '') or getattr(gold, 'original_text', '') or getattr(gold, 'text', '')
        summary = getattr(pred, prediction_field, '') or str(pred)
        rubric = getattr(gold, 'rubric', '') or RILE_TASK_CONTEXT

        # Score using ScoringOracle
...
def _create_rile_prediction_metric(
    scorer: Any,
    ...
    """Create metric for prediction mode (scoring single text, comparing to ground truth)."""

    def metric(gold, pred, trace=None, pred_name=None, pred_trace=None):
        # Get ground truth RILE
        ground_truth = getattr(gold, ground_truth_field, 0.0)
        if ground_truth is None:
            ground_truth = 0.0

        # Get text to score
        summary = getattr(pred, prediction_field, '') or getattr(pred, 'summary', '') or str(pred)

        # Get predicted RILE from scorer
        try:
            if hasattr(scorer, 'predict_rile'):
                predicted_rile, confidence, reasoning = scorer.predict_rile(summary)
            else:
                # Assume it's a callable that returns RILE directly
                result = scorer(summary)
                if isinstance(result, tuple):
                    predicted_rile = result[0]
                    reasoning = result[2] if len(result) > 2 else ""
                else:
                    predicted_rile = float(result)
                    reasoning = ""
        except Exception as e:
            # On error, return low score
            if with_feedback:
                return {'score': 0.0, 'feedback': f"Scorer error: {str(e)}"}
            return 0.0

        # Normalize RILE error to 0-1 score
        error = abs(predicted_rile - ground_truth)
        score = normalize_error_to_score(error, max_error=max_error)

        if with_feedback:
            feedback = f"RILE: predicted {predicted_rile:.0f}, expected {ground_truth:.0f}, error {error:.0f}"
...
def _create_rile_ground_truth_metric(
    ground_truth_field: str,
    max_error: float,
    with_feedback: bool,
) -> callable:
    """Create metric for ground truth mode (comparing two RILE scores, no text scoring)."""
    ...
    def metric(gold, pred, trace=None, pred_name=None, pred_trace=None):
        ...
        # Get prediction - try multiple field names
        predicted = getattr(pred, 'predicted_rile', None)
        if predicted is None:
            predicted = getattr(pred, 'rile', None)
        if predicted is None:
            # Try to parse label as float
        ...
        if with_feedback:
            feedback = f"RILE: {predicted:.0f} vs ground truth {ground_truth:.0f}, error {error:.0f}"
            return {'score': score, 'feedback': feedback}
        return score
...
def create_preservation_metric(scorer: Optional[Any] = None) -> callable:
    """
    Create metric for checking RILE preservation during summarization.

    This is used for auditing - comparing original text vs summary
    to ensure the summary preserves the RILE positioning.

    Args:
        scorer: RILESimilarityScorer or compatible. Created if None.

    Returns:
        DSPy-compatible metric function

    Example:
        metric = create_preservation_metric()
        score = metric(gold_example, pred_example)  # 0.0-1.0
    """
    return create_rile_metric(scorer, mode="similarity")


def create_summarizer_metric(
    classifier: Any,
    max_error: float = RILE_RANGE,
) -> callable:
    """
    Create metric for training summarizers to preserve RILE.

    This is used during summarization prompt optimization - the metric
    scores how well the generated summary preserves the document's RILE.

    Args:
        classifier: RILEOracleClassifier or compatible with predict_rile()
        max_error: Scale for error normalization. Default RILE_RANGE (200).

    Returns:
        GEPA-compatible metric function with feedback

    Example:
        metric = create_summarizer_metric(oracle_classifier)
        result = metric(gold, pred)  # {'score': 0.85, 'feedback': '...'}
    """
    return create_rile_metric(classifier, mode="prediction", max_error=max_error, with_feedback=True)


def create_evaluation_metric(max_error: float = RILE_RANGE) -> callable:
    """
    Create metric for evaluating RILE prediction accuracy.

    This is used for evaluation - comparing predicted RILE scores
    to ground truth without additional text scoring.
    ...
    Example:
        ...
        score = metric(gold_with_ground_truth, pred_with_predicted_rile)
    """
    return create_rile_metric(None, mode="ground_truth", max_error=max_error)
...
def create_rile_training_example(
    ...
    Example:
        ...
        # Use with create_rile_metric(scorer, mode="prediction")
    """
    fields = {
