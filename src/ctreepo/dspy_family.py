...
class DSPyFamilyConfig:
    ...
    #: Skip the per-call HF tokenization no-truncation guard. The guard holds the
    #: GIL, so under heavy thread fan-out (multi-doc audits) it serializes and
    #: starves the I/O-bound LM calls. Safe to disable for diagnostic/audit runs.
    skip_lm_input_budget_check: bool = False
    ...
    #: When enabled, optimizer-internal LM/adapter errors are fatal instead
    #: of being converted into failure-score rows. DSPy GEPA otherwise
    #: overrides the outer ``dspy.context(max_errors=...)`` with a high
    #: per-batch tolerance.
    strict_optimizer_errors: bool = False
    strict_optimizer_max_errors: int = 1
    ...
    def __post_init__(self) -> None:
        ...
        strict_max = int(self.strict_optimizer_max_errors)
        if strict_max < 1:
            raise ValueError(
                f"strict_optimizer_max_errors must be >= 1, got {self.strict_optimizer_max_errors!r}"
            )


@contextlib.contextmanager
def _strict_gepa_evaluate_errors(*, max_errors: int = 1):
    """Force DSPy GEPA's internal evaluator to fail on optimizer errors.

    DSPy 3.0.4 constructs GEPA evaluator batches with
    ``max_errors=len(batch) * 100``. That is useful for tolerant exploratory
    optimization, but invalid for conference-quality runs where parser,
    transport, or adapter failures change the objective. Patch only the GEPA
    utility module and restore it immediately after compile.
    """
    import dspy.teleprompt.gepa.gepa_utils as gepa_utils

    original_evaluate = getattr(gepa_utils, "Evaluate")
    strict_max = max(1, int(max_errors))

    class _StrictEvaluate(original_evaluate):  # type: ignore[misc, valid-type]
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            requested = kwargs.get("max_errors")
            if requested is None or int(requested) > strict_max:
                kwargs["max_errors"] = strict_max
            if kwargs.get("provide_traceback") is None:
                kwargs["provide_traceback"] = True
            super().__init__(*args, **kwargs)

    setattr(gepa_utils, "Evaluate", _StrictEvaluate)
    try:
        yield
    finally:
        setattr(gepa_utils, "Evaluate", original_evaluate)
...
class DSPyFamily(FamilyRuntime):
    ...
    def _assert_lm_input_budget(
        ...
        *,
        ...
        The per-call HF tokenization here holds the GIL, so under heavy
        thread fan-out (e.g. a multi-doc audit with hundreds of worker threads)
        it serializes and starves the I/O-bound LM calls. ``skip_lm_input_budget_check``
        disables it for diagnostic/audit runs where truncation is not a concern.
        """
        if bool(getattr(self.config, "skip_lm_input_budget_check", False)):
            return
        counts = {
    ...
    def _task_adapter(self):
        from src.ctreepo.problem_adapters import dspy_task_adapter
    ...
    def _apply_g(self, g_program: Any, *, prompt: str) -> str:
        ...
        try:
            ...
            if bool(getattr(self.config, "strict_optimizer_errors", False)):
                raise RuntimeError("DSPy g call failed") from exc
    ...
    def _batched_g_states_for_trees(
        self,
        *,
        g_program: Any,
        trees: Sequence[LabeledTree],
    ) -> List[Dict[str, str]]:
        """Compute trained-g states for every node of every tree, level-synchronously.

        All nodes at tree-level k — across ALL trees — are independent once
        level k-1 states exist, so each level is one parallel wave through the
        batch transport. The previous node-by-node walk serialized ~2N LM
        calls per document (hours for one deep doc at leaf=1); this keeps
        ``num_threads`` requests in flight regardless of document depth.
        Prompts, fallbacks, and state semantics are identical to the
        sequential walk in ``_reduce_tree_with_g``.
        """
        states_per_tree: List[Dict[str, str]] = [{} for _ in trees]
        tree_levels = [list(getattr(t, "levels", None) or []) for t in trees]
        max_height = max((len(lv) for lv in tree_levels), default=0)
        workers = max(1, int(self.config.num_threads or 1))
        for level in range(max_height):
            wave: List[Tuple[int, str, str, str]] = []
            for ti, tree in enumerate(trees):
                levels = tree_levels[ti]
                if level >= len(levels):
                    continue
                states = states_per_tree[ti]
                for node_id in levels[level]:
                    node = (
                        tree.get_node(str(node_id))
                    ...
                    wave.append((ti, str(node.node_id), prompt, fallback))
            if not wave:
                continue

            def run_wave_item(item: Tuple[int, str, str, str]) -> Tuple[int, str, str]:
                ti, node_id, prompt, fallback = item
                generated = str(self._apply_g(g_program, prompt=prompt) or "").strip()
                return ti, node_id, generated or fallback

            if len(wave) == 1 or workers == 1:
                results = [run_wave_item(item) for item in wave]
            else:
                with ThreadPoolExecutor(
                    max_workers=min(workers, len(wave))
                ) as pool:
                    results = list(pool.map(run_wave_item, wave))
            for ti, node_id, value in results:
                states_per_tree[ti][node_id] = value
        return states_per_tree
    ...
    def _full_tree_trace_with_g_states(
        ...
        *,
        g_program: Any,
        tree: LabeledTree,
        precomputed_states: Optional[Mapping[str, str]] = None,
        ...
        if precomputed_states is not None:
            states = dict(precomputed_states)
        elif g_program == self.TEACHER_PASSTHROUGH:
    ...
    def _compile(
        ...
        *,
        ...
        skip_threshold_raw = os.environ.get("TT_DSPY_SKIP_COMPILE_IF_BASE_SCORE_AT_LEAST", "")
        try:
            skip_threshold = float(skip_threshold_raw or 0.0)
        except ValueError:
            skip_threshold = 0.0
        if skip_threshold > 0.0:
            base_evalset = list(valset) if valset else list(trainset)
            original_base_eval_count = len(base_evalset)
            base_eval_max_raw = os.environ.get("TT_DSPY_BASE_SCORE_MAX_EXAMPLES", "")
            try:
                base_eval_max = int(base_eval_max_raw or 0)
            except ValueError:
                base_eval_max = 0
            if base_eval_max > 0 and len(base_evalset) > base_eval_max:
                if base_eval_max == 1:
                    indices = [0]
                else:
                    step = (len(base_evalset) - 1) / float(base_eval_max - 1)
                    indices = sorted({int(round(i * step)) for i in range(base_eval_max)})
                base_evalset = [base_evalset[i] for i in indices]
                LOGGER.info(
                    "Capped base DSPy program score evalset %d -> %d examples",
                    original_base_eval_count,
                    len(base_evalset),
                )
            if base_evalset:
                active_lm = getattr(dspy.settings, "lm", None)
                base_eval_threads = max(1, min(int(self.config.num_threads), len(base_evalset)))
                LOGGER.info(
                    "Evaluating base DSPy program before optimizer on %d examples with %d threads",
                    len(base_evalset),
                    base_eval_threads,
                )

                def score_base_example(example: Any) -> float:
                    ctx = (
                        dspy.context(lm=active_lm)
                ...
                if base_eval_threads == 1:
                    base_scores = [score_base_example(example) for example in base_evalset]
                else:
                    with ThreadPoolExecutor(max_workers=base_eval_threads) as pool:
                        base_scores = list(pool.map(score_base_example, base_evalset))
                base_score = float(sum(base_scores) / len(base_scores)) if base_scores else 0.0
                LOGGER.info(
                    "Base DSPy program score %.9f; skip threshold %.9f",
                    base_score,
                    skip_threshold,
                )
                if base_score >= skip_threshold:
                    LOGGER.info(
                        "Skipping DSPy optimizer compile because base score %.9f >= %.9f",
                        base_score,
                        skip_threshold,
                    )
                    return program
        ...
        def _run_optimizer_compile(compile_kwargs: Dict[str, Any]) -> Any:
            if optimizer_name == "gepa" and bool(self.config.strict_optimizer_errors):
                strict_max = int(self.config.strict_optimizer_max_errors)
                LOGGER.info(
                    "Using strict DSPy GEPA optimizer errors (max_errors=%d)",
                    strict_max,
                )
                with _strict_gepa_evaluate_errors(max_errors=strict_max):
                    return optimizer.compile(program, **compile_kwargs)
            return optimizer.compile(program, **compile_kwargs)

        def _cap_optimizer_examples(
            examples: Sequence[Any], *, env_name: str, label: str
        ) -> List[Any]:
            capped = list(examples)
            max_raw = os.environ.get(env_name, "")
            try:
                max_examples = int(max_raw or 0)
            except ValueError:
                max_examples = 0
            if max_examples > 0 and len(capped) > max_examples:
                original_count = len(capped)
                if max_examples == 1:
                    indices = [0]
                else:
                    step = (len(capped) - 1) / float(max_examples - 1)
                    indices = sorted({int(round(i * step)) for i in range(max_examples)})
                capped = [capped[i] for i in indices]
                LOGGER.info(
                    "Capped DSPy optimizer %s %d -> %d examples via %s",
                    label,
                    original_count,
                    len(capped),
                    env_name,
        ...
        compile_trainset = _cap_optimizer_examples(
            trainset, env_name="TT_DSPY_OPTIMIZER_TRAINSET_MAX_EXAMPLES", label="trainset"
        )
        compile_valset = _cap_optimizer_examples(
            valset, env_name="TT_DSPY_OPTIMIZER_VALSET_MAX_EXAMPLES", label="valset"
        )

        try:
            compile_kwargs = {
                "trainset": compile_trainset,
                "valset": compile_valset,
            }
            ...
            if bool(two_phase_evaluate) and optimizer_name == "mipro":
                ...
                try:
                    ...
                    return _run_optimizer_compile(compile_kwargs)
                finally:
                    mipro_optimizer_v2.Evaluate = old_evaluate
            return _run_optimizer_compile(compile_kwargs)
        except TypeError:
            return _run_optimizer_compile({"trainset": compile_trainset})
    ...
    def score_roots_with_f(
        ...
        *,
        ...
        batched_summaries: Optional[List[Optional[str]]] = None
        if (
            f_program != self.TEACHER_PASSTHROUGH
            and g_program not in (self.TEACHER_PASSTHROUGH, self.RAW_CONCAT)
        ):
            # Level-wave batched g across all trees (depth no longer serializes).
            wave_trees = [
                tree for tree in tree_list if list(getattr(tree, "levels", None) or [])
            ]
            wave_states = self._batched_g_states_for_trees(
                g_program=g_program, trees=wave_trees
            )
            states_by_id = {id(t): s for t, s in zip(wave_trees, wave_states)}
            batched_summaries = []
            for tree in tree_list:
                states = states_by_id.get(id(tree))
                if states is None:
                    # No levels: mirror _reduce_tree_with_g's degenerate path.
                    batched_summaries.append(self._reduce_tree_with_g(g_program, tree))
                    continue
                root = _root_node(tree)
                if root is not None:
                    batched_summaries.append(str(states.get(str(root.node_id)) or ""))
                elif states:
                    batched_summaries.append(next(reversed(states.values())))
                else:
                    batched_summaries.append(str(tree.document_text or ""))

        def score_tree(item: Tuple[int, LabeledTree]) -> Optional[float]:
            idx, tree = item
            if f_program == self.TEACHER_PASSTHROUGH:
                raw = _teacher_root_score(tree)
                return None if raw is None else float(raw)
            if batched_summaries is not None:
                summary = batched_summaries[idx]
            else:
        ...
        indexed = list(enumerate(tree_list))
        max_workers = max(1, min(len(indexed), int(self.config.num_threads or 1)))
        if max_workers == 1:
            return [score_tree(item) for item in indexed]
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            return list(pool.map(score_tree, indexed))
    ...
    def full_tree_traces_with_f_g(
        ...
        *,
        ...
        g_program = self._load_g_program(g)
        tree_list = list(trees)
        if g_program not in (self.TEACHER_PASSTHROUGH, self.RAW_CONCAT):
            # Level-wave batched g across all trees; skeleton assembly is cheap.
            wave_trees = [
                tree for tree in tree_list if list(getattr(tree, "levels", None) or [])
            ]
            wave_states = self._batched_g_states_for_trees(
            ...
            states_by_id = {id(t): s for t, s in zip(wave_trees, wave_states)}
            prepared = [
                self._full_tree_trace_with_g_states(
                    g_program=g_program,
                    tree=tree,
                    precomputed_states=states_by_id.get(id(tree)),
                )
                for tree in tree_list
            ]
        else:
            max_workers = max(
                1, min(len(tree_list), int(self.config.num_threads or 1))
            )
            if max_workers == 1:
                prepared = [
