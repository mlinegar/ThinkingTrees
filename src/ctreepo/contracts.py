...
# Objective contract is now the canonical upstream one. New specs serialize with
# the treepo schema ("treepo.objective.v1"); the historical "ctreepo.objective.v1"
# is still accepted on read for back-compat. See master plan Phase 2/6.
from treepo.objective import OBJECTIVE_SCHEMA_VERSION  # noqa: E402  ("treepo.objective.v1")
LEGACY_OBJECTIVE_SCHEMA_VERSION = "ctreepo.objective.v1"
...
# Estimator vocabulary + root/local resolver are sourced from the canonical
# upstream contract (`treepo.objective`) so there is a single definition of the
# objective estimator names and the root/local weight resolution. ThinkingTrees
# keeps the `ctreepo.objective.v1` serialization shape below for back-compat, but
# the vocabulary itself no longer lives here. See
# `docs/local_law_single_path_master_plan.md` (Phase 2/6).
from treepo.objective import (  # noqa: E402
...
def source_kind_for_tree_bundle_kind(kind: str) -> str:
    """Return the TreeBundle source_kind for a deprecated Manifesto bundle kind."""

    normalized = str(kind or "").strip().lower()
    if normalized == LEGACY_RAW_MANIFESTO_TOKEN_TREE:
        return SOURCE_KIND_RAW_INPUT
    if normalized == LEGACY_EXTERNAL_SUMMARY_TOKEN_TREE:
        return SOURCE_KIND_EXTERNAL_STATE
    raise ValueError(f"unknown legacy tree_bundle_kind: {kind!r}")
...
def source_kind_for_legacy_tree_text_source(source: str) -> str:
    """Return the TreeBundle source_kind for a deprecated Manifesto text source."""

    normalized = str(source or "").strip().lower()
    if normalized == "existing_summary":
        return SOURCE_KIND_EXTERNAL_STATE
    if normalized in {"", "aligned_text"}:
        return SOURCE_KIND_RAW_INPUT
    raise ValueError(f"unknown legacy tree_text_source: {source!r}")
...
class ObjectiveSpec:
    ...
    def __post_init__(self) -> None:
        ...
        # NOTE: full validation delegation to ``treepo.objective.ObjectiveSpec``
        # is intentionally deferred. treepo enforces "enabled estimator requires a
        # positive law component"; several live producers (e.g. masked law
        # packages in ``sim/composite_objective``) emit an enabled estimator with
        # an all-zero component mask, which treepo rejects. Adopting that
        # stricter check requires fixing the producers first (tracked in
        # ``docs/local_law_single_path_master_plan.md`` Phase 5/6). The estimator
        # vocabulary, root/local resolver, and serialization schema are already
        # the canonical treepo ones.
    ...
    *,
    ...
    if normalized["schema_version"] not in (
        ...
        errors.append(
            ...
            f"expected {OBJECTIVE_SCHEMA_VERSION} (or legacy "
            f"{LEGACY_OBJECTIVE_SCHEMA_VERSION}), found {normalized['schema_version']}"
        )
