...
from src.ctreepo.sim.core.law_stress_common import infer_law_stress_failure_reason
from src.ctreepo.sim.report.pdf_utils import safe_sem
    ...
    *,
    ...
    for key, group in sorted(groups.items()):
        ...
        if np.isfinite(leaf) and np.isfinite(merge) and np.isfinite(spread):
            from src.ctreepo.sim.report.data_loading import _law_score
