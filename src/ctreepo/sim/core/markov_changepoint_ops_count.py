...
# ---------------------------------------------------------------------------
from src.ctreepo.sim.core.markov_baselines import (  # noqa: E402
    _rf_doc_features,
...
def run_markov_changepoint_ops_count_experiment(
    ...
    *,
    ...
    if train_prepped:
        ...
        if _is_fno:
            ...
            def _resolve_oracle_metric_from_config(cfg: OPSCountConfig):
                ...
                if not _om_name:
                    return None
                from src.ctreepo.sim.core.oracle_metric import resolve_oracle_metric
                import src.ctreepo.sim.core.markov_oracle_metric  # noqa: F401 — register
    ...
    if bool(config.include_deeponet_baseline):
        from src.ctreepo.sim.core.markov_neural_operator_baselines import _fit_deeponet_baseline
        (deeponet_train, deeponet_val, deeponet_test, deeponet_fit) = _fit_deeponet_baseline(
            config=config, seeds=seeds, device=device,
            train_docs=docs_train, val_docs=docs_val, test_docs=docs_test,
        )
    if bool(config.include_mlp_bigram_baseline):
        from src.ctreepo.sim.core.markov_neural_operator_baselines import _fit_mlp_bigram_baseline
    ...
    if bool(config.include_cnn1d_baseline):
        from src.ctreepo.sim.core.markov_neural_operator_baselines import _fit_cnn1d_baseline
