...
def run_tensor_lda_book_weight_benchmark(
    ...
    for b in range(int(config.n_books_test)):
        ...
        truth_root = _normalize_simplex_vec(np.mean(truth_leaf, axis=0))
        ...
        # Policy 5: oracle tree upper bound.
        root_l1["ctree_oracle"].append(0.0)
        root_l2["ctree_oracle"].append(0.0)
        latent_l1["ctree_oracle"].append(_l1(truth_root, latent_root))
        c1_err["ctree_oracle"].extend([0.0] * truth_leaf.shape[0])
        # Each merge in a full binary reduction contributes one C3 check; `C-1` is exact.
        c3_err["ctree_oracle"].extend([0.0] * max(0, truth_leaf.shape[0] - 1))
        q_leaf["ctree_oracle"].append(float(truth_leaf.shape[0]))
        q_internal["ctree_oracle"].append(float(max(0, truth_leaf.shape[0] - 1)))
