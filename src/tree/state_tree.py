        ...
        metadata.update(
            {
                ...
                # None-safe: nodes can be unsupervised (score=None) under mixed
                # node/root supervision (e.g. NA chunks); the trace target is
                # reference-only, so default missing scores to NaN rather than crash.
                "target": (
                    float(_sc)
                    if (_sc := getattr(source_node, "score", 0.0)) is not None
                    else float("nan")
                ),
            }
        )
