...
OPSNode = Node
OPSTree = Tree
create_leaf_node = leaf
create_internal_node = node
...
def build_tree_from_leaves(
    ...
    """
    Build a tree bottom-up from leaf nodes.

    DEPRECATED: Use TreeBuilder from src.ops_engine.builder instead.

    Args:
        leaves: List of leaf nodes to build from
        summarizer: Function(content, rubric) -> summary
        rubric: Preservation criteria for summarization

    Returns:
        Complete Tree with all levels built
    """
    if not leaves:
    ...
    while len(current_level) > 1:
        ...
        for i in range(0, len(current_level), 2):
            if i + 1 < len(current_level):
                ...
                summary = summarizer(combined_content, rubric)
                parent = node(left, right, summary)
    ...
    return Tree(root=current_level[0], rubric=rubric)
