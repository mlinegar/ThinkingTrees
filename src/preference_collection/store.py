"""
PreferenceStore -- manages pending requests and completed responses.

Bridges the preference API to the existing ReviewQueue infrastructure.
...
from src.preference_collection.types import (
    PreferenceRequest,
    PreferenceResponse,
    binary_projection_dataset_from_responses,
...
class PreferenceStore:
    """In-memory + file-backed store for preference requests and responses.
    ...
    def __init__(
        ...
        self._pending: Dict[str, PreferenceRequest] = {}
        self._completed: Dict[str, Tuple[PreferenceRequest, PreferenceResponse]] = {}
        if self.storage_path is not None and bool(load_existing) and self.storage_path.exists():
            self.load(self.storage_path)

    # --- Enqueue / Submit ---

    def enqueue(self, request: PreferenceRequest) -> str:
        """Add a preference request to the pending queue.
        ...
        logger.debug("Enqueued preference request: %s", request.request_id)
        return request.request_id

    def submit(self, request_id: str, response: PreferenceResponse) -> bool:
        ...
        logger.debug("Submitted preference response for: %s", request_id)
        return True
    ...
    def get_request(self, request_id: str) -> Optional[PreferenceRequest]:
    ...
    def get_completed(
        self,
        limit: int = 100,
    ) -> List[Tuple[PreferenceRequest, PreferenceResponse]]:
    ...
    def to_preference_dataset(self):
        """Export completed items as the unified treepo PreferenceDataset."""
        with self._lock:
            items = list(self._completed.values())
        return preference_dataset_from_responses(items)

    def to_supervision_dataset(self):
        """Export completed human/LLM/oracle responses as canonical supervision."""
        with self._lock:
            items = list(self._completed.values())
        return supervision_dataset_from_responses(items)
    ...
    def to_binary_projection_dataset(
        ...
        *,
        ...
        """Export completed responses as a canonical binary optimizer projection."""
        with self._lock:
            items = list(self._completed.values())
        return binary_projection_dataset_from_responses(items, projection=projection)
    ...
    def save(self, path: Optional[Path] = None) -> None:
        ...
        if resolved is None:
            raise ValueError("PreferenceStore.save requires a path")
        path = Path(resolved)
        path.parent.mkdir(parents=True, exist_ok=True)
        with self._lock:
            data = self._snapshot_unlocked()
            self._write_snapshot_atomic(path, data)
        logger.info("Saved preference store to %s", path)

    def load(self, path: Optional[Path] = None) -> None:
        """Load store state from JSON file (merges with existing)."""
        resolved = path or self.storage_path
        if resolved is None:
            raise ValueError("PreferenceStore.load requires a path")
        path = Path(resolved)
        if not path.exists():
            return
        ...
        with self._lock:
            for rid, req_data in data.get("pending", {}).items():
                if rid not in self._pending and rid not in self._completed:
                    self._pending[rid] = PreferenceRequest.from_dict(req_data)
            for rid, item_data in data.get("completed", {}).items():
                if rid not in self._completed:
                    req = PreferenceRequest.from_dict(item_data["request"])
                    resp = PreferenceResponse.from_dict(item_data["response"])
                    self._completed[rid] = (req, resp)
                self._pending.pop(rid, None)
        logger.info("Loaded preference store from %s", path)
    ...
    def submit_human_pairwise_preference(
        self,
        request_id: str,
        *,
        preferred: str,
    ...
    def submit_human_scalar_preference(
        self,
        request_id: str,
        *,
        score: float,
        ...
        response = PreferenceResponse.from_human_scalar_preference(
            request_id=request_id,
            score=score,
    ...
    def import_from_review_queue(self) -> int:
        ...
        for item in self.review_queue.get_unreviewed_items():
            request = PreferenceRequest.from_flagged_item(item)
    ...
    def _bridge_to_review_queue(self, request: PreferenceRequest) -> None:
    ...
    def _bridge_response_to_review_queue(
        ...
        """Update ReviewQueue item with the preference response."""
        if self.review_queue is None:
            return
        ...
        with self._file_lock(path):
            if merge_existing and path.exists():
                try:
                    ...
                    logger.warning("Could not merge existing preference store snapshot at %s", path)
