"""
PreferenceCollector protocol and registry.

Generalizes PreferenceDeriver for type-agnostic preference collection.
The protocol supports pairwise comparison, scalar rating, written critique,
and arbitrary combinations through the PreferenceRequest/PreferenceResponse pair.
...
Usage:
    from src.preference_collection import get_collector, register_collector, PreferenceRequest
    ...
    # Collect preferences
    request = PreferenceRequest(request_id="r1", text_a="...", text_b="...", rubric="...")
...
# =============================================================================
# PreferenceCollector Protocol
# =============================================================================

@runtime_checkable
class PreferenceCollector(Protocol):
    """Generalized preference collection protocol.

    Subsumes PreferenceDeriver for pairwise tasks, and extends to
    scalar rating, written critique, and arbitrary preference data.

    Implementations inspect ``request.dimensions`` to determine what
    preference data to provide. They may provide more dimensions than requested
    ...
    def collect(
        ...
        **kwargs: Any,
        ...
        """Collect preference data for the given request.

        Args:
            request: PreferenceRequest with context and requested dimensions
            **kwargs: Collector-specific arguments

        Returns:
            PreferenceResponse with collected preference data
        """
        ...
...
def register_collector(name: str):
    """Decorator to register a preference collector class.
...
def get_collector(name: str, **kwargs: Any) -> PreferenceCollector:
    """Get a preference collector by name.
...
class PreferenceDeriverAdapter:
    """Wraps any existing PreferenceDeriver as a PreferenceCollector.

    Existing derivers (JudgeDeriver, GenRMDeriver, OracleDeriver) work through
    this adapter without modification.
    ...
    def collect(
        ...
        **kwargs: Any,
        ...
        """Collect pairwise preference by delegating to the wrapped deriver.
