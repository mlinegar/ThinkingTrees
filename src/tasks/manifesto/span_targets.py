...
def parse_compact_scores_json(value: Any) -> dict[str, float]:
    ...
    if isinstance(value, Mapping):
        ...
        if isinstance(raw, str):
            return parse_compact_scores_json(raw)
    if "scores" in raw and isinstance(raw.get("scores"), Mapping):
        raw = raw["scores"]
    # Some local models (e.g. DiffusionGemma) flatten the nested path into a
    # single dotted key instead of real nesting:
    #   {"cmp_state.compact_targets": {...}}  or  {"compact_targets.<dim>": v}.
    # Accept the dotted-container form so a well-formed-but-flat state is not
    # discarded as "missing all dimensions".
    for dotted in ("cmp_state.compact_targets", "compact_targets"):
        if dotted in raw and isinstance(raw.get(dotted), Mapping):
            return parse_compact_scores_json(raw[dotted])
    ...
    # Lossless structural normalization ONLY. If the state is not in canonical
    # form (numeric dims under cmp_state/compact_targets, possibly dotted), we
    # return {} and the caller defers to the f LLM readout to interpret it.
    # No value heuristics here (no word->number, no domain-key guessing, no
    # abstain keyword list): interpretation is the LLM's job, not the parser's.
    return out
